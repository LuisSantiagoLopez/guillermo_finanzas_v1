...
for i in range(indeval_grande.shape[0]):
    ...
    reducir_por_serie = emisora_vector[emisora_vector['SERIE'] == serie]
    tasa_T = reducir_por_serie['TASA DE RENDIMIENTO'].iloc[0]
    sobretasa_T = reducir_por_serie['SOBRETASA'].iloc[0]
    
    
    ## EXTRAEMOS CALIFICACIONES Y FECHAS DE VCTO
    mdys = reducir_por_serie['MDYS'].iloc[0].replace('.mx','')
    s_and_p = reducir_por_serie['S&P'].iloc[0].replace('.mx','')
    fecha_vcto = reducir_por_serie['FECHA VCTO'].iloc[0]
    fecha_vcto = datetime.strptime(fecha_vcto, '%Y-%m-%d').date()
